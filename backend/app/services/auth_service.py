...
async def get_valid_token(user_id: str, provider: str) -> Optional[str]:
    ...
    # If no expiration is set, assume it's valid (or if we just lack refresh token)
...
async def _refresh_token(provider: str, refresh_token: str) -> Optional[Dict[str, Any]]:
    ...
    async with httpx.AsyncClient() as client:
        if provider == "spotify":
            ...
            res = await client.post("https://accounts.spotify.com/api/token", data=data, auth=auth)
        ...
        if res.status_code == 200:
            return res.json()
        print(f"Failed to refresh {provider} token: {res.text}")
        return None
